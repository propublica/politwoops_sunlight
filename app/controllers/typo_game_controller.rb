class TypoGameController < ApplicationController
  def index
    ...
    @latest_tweets = Tweet.with_content.where(:deleted => 0, :politician_id => @tweet.politician_id).where("created > ?", @tweet.modified).reorder(:created).reverse_order.limit(5).all
  end
